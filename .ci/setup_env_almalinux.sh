...
if [ -f /etc/os-release ]; then
  alma_version=$(grep VERSION_ID /etc/os-release | cut -d '"' -f2)
else
  alma_version=$(grep VERSION_ID /usr/lib/os-release | cut -d '"' -f2)
fi

[ "${alma_version:0:2}" == "8." ] || die "This script is for Alma 8 only"

# Send error when a package is not available in the repositories
if ! grep "skip_missing_names_on_install=0" /etc/yum.conf; then
...
# Enable priority to Alma Base repo in order to
# avoid perl updating issues
for repo_file_path in /etc/yum.repos.d/almalinux.repo ; do
    if [ -f "$repo_file_path" ]; then
	repo_file="$repo_file_path"
	break
    fi
done
[ -n "${repo_file:-}" ] || die "Unable to find the AlmaLinux base repository file"

if ! grep "priority=1" ${repo_file}; then
...
main()
{
	...
	# On alma:8 container image the installation of coreutils
